...
public class MemberAuth extends HttpServlet{
	...
	/*
	 클라이언트가 최초로 요청했을 때 서블릿 객체가 생성되는데 이때 최초
	 1번만 호출도는 init()에서 DB연결 처리를 한다.
	 */
	@Override
	public void init() throws ServletException {
		/*
		 서블릿 내에서 application 내장객체를 얻어온다. 
		 모델2 방식에서는 서블릿이 먼저 요청을 받기 때문에 모델1 방식과 
		 같이 JSP에서 매개변수로 내장객체를 전달할 수 없다. 
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp) 
		...
		if (memberName != null) {
			/*
			 DTO에 회원의 이름이 저장되어 있다면 인증에 성공한 것으로 판단한다.
			 */
			...
			if (admin_id.equals(id))
				req.setAttribute("authMessage", admin_id + "는 최고 괸리자 입니다.");
			else
				req.setAttribute("authMessage", "귀하는 회원이 아닙니다.");
		}
		/*
		 앞에서 판단한 인증정보는 request영역에 저장한 후 JSP로 포워드 한다.
		 */
