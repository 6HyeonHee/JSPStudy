...
저장할 Map을 생성한다. 파라미터가 추가되는 경우에도 그대로 활용할 수 있다.
*/
Map<String, Object> param = new HashMap<String, Object>();

/*
검색폼에서 입력한 검색어와 필드명을 파라미터로 받아온다.
<form> 태그의 전송방식은 get, action 속성은 없는 상태이므로 
현재 페이지로 폼값이 전송된다.
*/
...
if (searchWord != null) {
	...
	검색어를 추가한다. 이 값은 DB 처리를 위한 DAO로 전달된다.
	*/
...
/*
전체 페이지수를 계산한다.
(전체 게시물 수 / 페이지당 게시물 수) => 결과값의 올림처리
Ex) 108 / 10 = 10.8 => 올림처리하여 11페이지로 계산
*/
int totalPage = (int)Math.ceil((double)totalCount / pageSize);

/*
목록에서 처음 진입했을 때는 페이지 관련 파라미터가 없는 상태이므로
1page로 지정한다. 만약 파라미터 pageNum이 있다면 request 내장객체를
통해 받아온 후 페이지 번호로 지정하면 된다.
*/
int pageNum =1;
