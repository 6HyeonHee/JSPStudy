...
// 아래와 같이 타입매개변수를 생략해도 Object기반의 List가 된다.(코드추가)
...
%>
<ul>
	<!-- 첫번째는 String 인스턴스가 출력된다. List는 배열의 특성을 가지므로
	인덱스로 접근할 수 있다. -->
	<li>0번째 요소 : ${ Ocean[0] }</li>
	<!-- Person 인스턴스를 출력한다. 멤버변수명을 통해 getter()를 호출하여 출력한다.
...
<ul>
	...
	<!-- 단, 한글인 경우에는 .(닷)을 사용할 수 없다. 파싱할 수 없다는 에러가 발생된다. -->
	<li>한글 Key : ${ King["한글"] }, ${ King['한글'] }, \${ King.한글  }</li>
	<!-- 
	EL식 앞에 \(역슬러쉬)를 붙이면 주석이 된다. 단 코드가 숨겨지는 것은 아니고
	화면에 작성한 그대로 출력된다. 실행되지 않는 코드이므로 주석이라 표현한다.
	 -->
</ul>
</body>
</html>