...
속성 정리
 ...
 -->
	...
	<!-- 
	if(number % 2 == 0) 와 동일한 조건의 if문으로 해당 조건의 결과가 result에
	저장된다. 나머지는 0이므로 true가 된다.
	 -->
	...
	<!-- JSTL의 if태그는 else구문이 별도로 없으므로 if문과 반대의 조건을
	만들어서 두개의 if태그를 사용한다. -->
	<!-- 
	Java에서는 비교연산자 == 과 equals()가 서로 다르지만, EL에서는 eq를 통해
	값에 대한 비교와 문자열에 대한 비교 모두를 할 수 있다.
	 -->
	 <!-- 첫번째 if문은 false의 결과가 나온다 따라서 result2에는 flase가 저장된다. -->
	<c:if test="${ string eq 'Java' }" var="result2">
		문자열은 Java입니다. <br />
	</c:if>
	<!-- result2에 not을 붙여 반대의 조건을 만든다. 따라서 else와 같은 구문이된다. -->
	...
	<c:if test="${ not empty param.user }">
		...
		if 태그는 else 구문이 별도로 없으므로 not을 통해 else와 같은 구문을
		구성해야한다.
		 -->
		<c:if test="${ param.user == 'tjoeun' && param.pass eq '1234' }" var="loginResult">
			${ param.user }님, 하이룽~ <br />
		</c:if>
